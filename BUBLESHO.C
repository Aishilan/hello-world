...
void main()
{
int gmode=DETECT,gdriver,area,*burst,*arrow,*bow;
int maxx,maxy;
int p=400,q=300,m=100,n=100,x=m,y=n,key,score=0,finish=0,level=1,l_flag=1;
char score1[15],ch,cnt_ball[15],char_level[2];
 ...
 *bow= malloc(area);
 getimage(x+25,y-65,x+66,y+95,bow);
 ...
 while(!finish)
 {
	...
	if(kbhit())
	{
		...
		if(count_arrow>0 && count_balloon>0)
		{
			...
			if(level==1)
			outtextxy(getmaxx()/2-220,getmaxy()/2+20,"YOU REQUIRE TO PRACTISE MORE");
			if(level==2)
	...
	void start()
	{
		...
		outtextxy(10,190,"4.You score 20 points everytime you shoot the balloon");
		outtextxy(10,230,"5.First level has 6 arrows and 10 balloons");
		outtextxy(10,270,"6.You require 100 points to enter the second level");
		outtextxy(10,310,"7.Second level has 6 arrows and 8 balloons");
		outtextxy(10,350,"8.You require 200 points to enter the third level");
